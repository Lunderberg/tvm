...
class SeqExpr : public Expr {
 public:
   ...
   * provides backwards compatibility for callsites that pass a
   * `relax::Expr` into constructors that expect a `relax::SeqExpr`.
   * No copy is performed if the expression is already a
   * `relax::SeqExpr`.
   */
  TVM_DLL SeqExpr(Expr body, Span span = Span());  // NOLINT(*)
