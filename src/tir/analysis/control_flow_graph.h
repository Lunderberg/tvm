...
class ControlFlowGraph {
 ...
 private:
  ...
  struct ControlFlowBlock {
     ...
     * If this contains a `tir::if_then_else` node, the conditions
     * will be hoisted out of the expression and treated as
     * independent accesses.
     *
     * \returns The newly generated BufferTouch points
     */
    std::vector<BufferTouch> MakeBufferTouch(ControlFlowGraph* graph, const Buffer& buf,
     ...
     * \param known_expr_value The value being written to the buffer
     *
     * \returns The newly generated BufferTouch points, and a map specifying
     * all free parameters that may occur in the BufferTouch's
     * predicate.
     */
    std::pair<std::vector<BufferTouch>, Map<Var, Range>> MakeBufferTouch(
