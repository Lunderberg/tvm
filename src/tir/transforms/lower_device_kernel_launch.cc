...
class DeviceInfoCollector : public StmtVisitor {
 public:
  static DeviceInfo Collect(const PrimFuncNode* func) {
    ...
    collector.info_.target = func->GetAttr<Target>(tvm::attr::kTarget).value();
...
class DeviceKernelMutator : public StmtExprMutator {
 public:
  ...
  explicit DeviceKernelMutator(std::unordered_map<const GlobalVarNode*, DeviceInfo> device_info_map)
    ...
    for (const auto& [gvar, info] : device_info_map_) {
      ICHECK(!info.target->GetHost().defined())
          << "tir.LowerDeviceKernelLaunch expects empty target_host.  "
          << "Please run tir.SplitHostDevice first.";
    }
  }

  PrimFunc RewriteKernelLaunchSite(PrimFunc func) {
    ICHECK(!current_target_.defined());
    current_target_ = func->GetAttr<Target>(tvm::attr::kTarget).value();
  ...
  PrimFunc UpdateKernelAttributes(const GlobalVar& gvar, PrimFunc func) const {
    if (device_kernel_launch_.count(gvar.get())) {
      func =
          WithAttr(func, tvm::attr::kCallingConv, Integer(tvm::CallingConv::kDeviceKernelLaunch));
 ...
 private:
  PrimExpr VisitExpr_(const CallNode* op) {
    ...
    return Call(node->dtype, builtin::tvm_call_packed(), call_args);
  }
...

    {
      ...
      for (const auto& [gvar, base_func] : mod->functions) {
        if (auto* ptr = base_func.as<PrimFuncNode>()) {
          auto prim_func = mutator.RewriteKernelLaunchSite(GetRef<PrimFunc>(ptr));
