 ...
 * \file tir/transforms/lower_tvm_stack_parameters.cc
 */
...
class StackParametersLower : public StmtExprMutator {
 public:
...
Pass LowerTVMStackParameters() {
  auto pass_func = [](PrimFunc f, IRModule m, PassContext ctx) {
    auto* n = f.CopyOnWrite();
    n->body = StackParametersLower().Build(n->body);
    VLOG(2) << "LowerTVMStackParameters: " << f;
    return f;
  };
  return CreatePrimFuncPass(pass_func, 0, "tir.LowerTVMStackParameters", {});
}

TVM_REGISTER_GLOBAL("tir.transform.LowerTVMStackParameters")
    .set_body_typed(LowerTVMStackParameters);
