...
void ArgBinder::BindDLTensor(const Buffer& buffer, const PrimExpr& device_type,
  ...
  ICHECK(buffer->pre_flattened_buffer)
  ...
  auto pre_flattened_buffer = buffer->pre_flattened_buffer.value();
  ...
  PrimExpr a_ndim =
      make_const(tvm_ndim_type, static_cast<int64_t>(pre_flattened_buffer->shape.size()));
  std::ostringstream ndim_err_msg;
  ndim_err_msg << arg_name << ".ndim is expected to equal " << pre_flattened_buffer->shape.size();
  auto msg = tvm::tir::StringImm(ndim_err_msg.str());
  asserts_.emplace_back(AssertStmt(a_ndim == v_ndim, msg, nop));
  // type checks
  std::ostringstream type_err_msg;
  type_err_msg << arg_name << ".dtype is expected to be " << pre_flattened_buffer->dtype;
  PrimExpr cond = (TVMArrayGet(DataType::UInt(8), handle, builtin::kArrTypeCode) ==
                       IntImm(DataType::UInt(8), pre_flattened_buffer->dtype.code()) &&
                   TVMArrayGet(DataType::UInt(8), handle, builtin::kArrTypeBits) ==
                       IntImm(DataType::UInt(8), pre_flattened_buffer->dtype.bits()) &&
                   TVMArrayGet(DataType::UInt(16), handle, builtin::kArrTypeLanes) ==
                       IntImm(DataType::UInt(16), pre_flattened_buffer->dtype.lanes()));
  if (!(pre_flattened_buffer->dtype == DataType::Int(4) ||
        pre_flattened_buffer->dtype == DataType::UInt(4) ||
        pre_flattened_buffer->dtype == DataType::Int(1))) {
  ...
  // shape field
  Buffer buf_shape = decl_buffer({IntImm(DataType::Int(32), pre_flattened_buffer->shape.size())},
  ...
  for (size_t k = 0; k < pre_flattened_buffer->shape.size(); ++k) {
    if (pre_flattened_buffer->dtype == DataType::Int(4) ||
        pre_flattened_buffer->dtype == DataType::UInt(4) ||
        pre_flattened_buffer->dtype == DataType::Int(1)) {
      break;
    }
    Bind_(pre_flattened_buffer->shape[k],
          cast(pre_flattened_buffer->shape[k].dtype(),
               BufferLoad(buf_shape, {IntImm(DataType::Int(32), k)})),
          shape_element_name(k), true);
  }
  // strides field
  Buffer buf_strides =
      decl_buffer({IntImm(DataType::Int(32), pre_flattened_buffer->strides.size())}, tvm_shape_type,
                  arg_name + ".strides");
  ...
  if (pre_flattened_buffer->strides.size() == 0) {
    ...
    for (size_t i = pre_flattened_buffer->shape.size(); i != 0; --i) {
      size_t k = i - 1;
      PrimExpr value = cast(pre_flattened_buffer->shape[k].dtype(),
                            BufferLoad(buf_strides, {IntImm(DataType::Int(32), k)}));
      value = tvm::if_then_else(v_strides_is_null, stride, value);
      value = tvm::if_then_else(pre_flattened_buffer->shape[k] == 1, 0, value);
      Bind_(pre_flattened_buffer->strides[k], value, stride_element_name(k), true);
      stride = analyzer_.Simplify(stride * pre_flattened_buffer->shape[k]);
    }
  } else {
    PrimExpr stride_from_shape = 1;

    for (int k = pre_flattened_buffer->strides.size() - 1; k >= 0; k--) {
      PrimExpr explicit_stride = cast(pre_flattened_buffer->shape[k].dtype(),
                                      BufferLoad(buf_strides, {IntImm(DataType::Int(32), k)}));

      Bind_(pre_flattened_buffer->strides[k],
            tvm::if_then_else(v_strides_is_null, stride_from_shape, explicit_stride),
            stride_element_name(k), true);

      stride_from_shape *= cast(pre_flattened_buffer->shape[k].dtype(),
                                BufferLoad(buf_shape, {IntImm(DataType::Int(32), k)}));
    }
  }
  // Byte_offset field.
