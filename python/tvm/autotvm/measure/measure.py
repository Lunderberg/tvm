...
class Builder(object):
    ...
    def build(self, measure_inputs):
        ...
        Returns
        -------
        build_results: Context manager for list of BuildResult

            A context manager (e.g. for managing temporary files
            generated by the builder) whose __enter__ method returns
            the build results.  Usage of the build results should be
            inside a with block.
...
def create_measure_batch(task, option):
    ...
    def measure_batch(measure_inputs):
        with builder.build(measure_inputs) as build_results:
