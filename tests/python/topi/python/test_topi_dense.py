...
def test_dense(
    batch_size, in_dim, out_dim, use_bias, in_dtype, out_dtype, dense_ref_data, target, dev
):
    ...
    Fallback = Int8Fallback if in_dtype == "int8" else contextlib.nullcontext
    implementations = tvm.topi.testing.dispatch(target, _dense_implementation[in_dtype])
